-- ============================================================================
-- Update Cost Reduction Window to 60-90 Days
-- Migration: 20250110000002_update_cost_reduction_window.sql
-- Description: Changes cost reduction recommendations from 7 days to 60-90 days
--              to allow proactive planning and actual sales/redistribution time
-- ============================================================================

-- Update the generate_ai_recommendations function
...
BEGIN
  ...
  LOOP
    ...
    IF p_recommendation_type IS NULL OR p_recommendation_type = 'inventory_analysis' THEN
      ...
      FROM public.stock_items si
      WHERE si.branch_id = v_branch_id
        AND (si.quantity * si.unit_price) > 10000
        AND si.status = 'active';
    ...
    IF p_recommendation_type IS NULL OR p_recommendation_type = 'cost_reduction' THEN
      ...
      FROM public.stock_items si
      WHERE si.branch_id = v_branch_id
        AND si.expiry_date BETWEEN NOW() + INTERVAL '60 days' AND NOW() + INTERVAL '90 days'
        AND si.status = 'active';
    ...
    IF p_recommendation_type IS NULL OR p_recommendation_type = 'stock_optimization' THEN
      SELECT 
        COUNT(*) FILTER (WHERE si.quantity > 100),
        COUNT(*) FILTER (WHERE si.quantity < 5)
...
COMMENT ON FUNCTION public.generate_ai_recommendations IS 'Generates AI-powered recommendations based on stock data analysis. Cost reduction recommendations now focus on items expiring in 60-90 days to allow proactive planning.';
